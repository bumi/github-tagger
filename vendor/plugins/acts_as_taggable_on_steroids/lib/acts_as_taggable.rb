      ...
      module SingletonMethods
        ...
        def find_options_for_find_tagged_with(tags, options = {})
            ...
            if options.delete(:match_all)
              conditions << <<-END
                (SELECT COUNT(*) FROM #{Tagging.table_name}
                 INNER JOIN #{Tag.table_name} ON #{Tagging.table_name}.tag_id = #{Tag.table_name}.id
                 WHERE repository_id = #{table_name}.id AND
          ...
          { :select => "DISTINCT #{table_name}.*",
            :joins => "INNER JOIN #{Tagging.table_name} #{taggings_alias} ON #{taggings_alias}.repository_id = #{table_name}.#{primary_key} " +
        ...
        def find_options_for_tag_counts(options = {})
          ...
          joins << "INNER JOIN #{table_name} ON #{table_name}.#{primary_key} = #{Tagging.table_name}.repository_id"
